...
for i = 1:1:N + 1
    sfbar(i) = dsbar * (i-1); % originally i-1, but changed to i because etafluv devided by 0 ; 
    rfluv(i) = rsi * sfbar(i);
...
%     Inner loop
     for w = 1:1:Ntoprint
        ...
        for i = 1:1:N+1
            ts(i) = Cfa * (qw / Hfluv(i)) ^ 2 / (Rs * g * Ds);
            if ts(i) <= tsc 
                qs(i) = 0;
            else
                qs(i) = (Rs * g * Ds) ^ 0.5 * Ds * al * (ts(i) - tsc) ^ nl;
            end
            qs=qs';
...
%        rsdot(j,1) = 1 / Sa * (qs(N + 1)*(1+Lamms) / ((1 - lps) * (rbb - rss))); %1d S_s dot (52) :shock condition
          rsdot(j,1)= 1/Sa * (-etasdot(j,1)+intM*Bc*qs(N+1)*(1+Lamms)/0.5/thetaF/(1-lps)/(rbb^2-rss^2)); % (97) : 2d shock condition
        %-1/Sa*xiddot+
...
%         end
        ...
        for i = 1:1: N + 1
            ...
            if i==1
            etafluv(i) = etafluv(i) + (dt * (-qsdev(i,1) / rss / (1 - lps)*(1+Lamms)*intM)) + (sfbar(i) * rsdot(j,1) / rss * etafdev(i,1));
            else
            etafluv(i) = etafluv(i) + dt * ((sfbar(i)*rsdot(j,1)/rss*etafdev(i,1)) -(Bc/rss*qsdev(i,1)*intM/(1-lps)*(1+Lamms)/thetaF/rss/sfbar(i)));  % 3D fluvial Exner (91)
            end
...
%         Lamms=lambda;
        ...
        for i = 1:1: Ns + 1  %  to line 215 : etatdev & cibar dqm 
            jj = N + 1 + i;
            
            if i == Ns + 1
                etatdev(i) = (etaturb(i) - etaturb(i - 1)) / dstbar;
                cibardev(i)=(cibar(i)-cibar(i-1))/dstbar;
            else
                if i == 1
                    etatdev(i) = (etaturb(i + 1) - etaturb(i)) / dstbar;
                    cibardev(i)=(cibar(i+1)-cibar(i))/dstbar;
                    ...
                    cibardev(i)=(cibar(i+1)-cibar(i))/dstbar;
                            ...
                            Qm_ghost(1)=qmo*Bc; %L3/T
%                             qm(1)=qms(N+1)*(rbb-rt)/(rss-rt);
                            coef(i)=rbdot(j,1)*(1-stbar(i))/(rmax-rbb);
                            
        end
        
        for i = 1 : 1 : Ns+1
                            %% cibar volume ratio
                    % dqm needs dt ? ? ? or not   // dsbar -> ((rmax-rbb)/Ns)                                

            if i == 1 % dt :  0.00025 * timeyr  ,  too large ?  when dt : 0.00025 is better
%                 cibar(i) = cibar(i) + dt*(((Qm_ghost(1)/thetaF)-(rturb(1)*cibar(1)*Hturb(1)*Uturb(1))-(vsm*ro*cibar(1)*((rmax-rbb)/Ns)*(rturb(1)+rturb(2))/2) )/ (Hturb(1)*((rmax-rbb)/Ns)*(rturb(1)+rturb(2))/2));
%                 cibar(i) = cibar(i) + dt*( rbdot(j,1)*(1-stbar(i))*cibardev(i)/(rmax-rbb))... 
%                 +dt*(((rturb(1)*Qm_ghost(1))-(rturb(1)*cibar(1)*Hturb(1)*Uturb(1))-(vsm*ro*cibar(1)*((rmax-rbb)/Ns)*(rturb(1)+rturb(2))/2) )/ (Hturb(1)*((rmax-rbb)/Ns)*(rturb(1)+rturb(2))/2));

                    cibar(i) = cibar(i) +  (rbdot(j,1)*(1-stbar(i))*cibardev(i)/(rmax-rbb))*dt+...
                                            dt/Hturb(i)*(-vsm*ro*cibar(i)-(cibar(i)*Uturb(i)*Hturb(i)-qmo)/((rmax-rbb)*dstbar));
                            % cibar(residual_) = input - output - deposition
                            
            else
%                 cibar(i) = cibar(i) + dt*(((rturb(i)*cibar(i-1)*Uturb(i-1)*Hturb(i-1))-(rturb(i+1)*cibar(i)*Hturb(i)*Uturb(i))-(vsm*ro*cibar(i)*((rmax-rbb)/Ns)*(rturb(i)+rturb(i+1))/2)) / (Hturb(i)*((rmax-rbb)/Ns)*(rturb(i)+rturb(i+1))/2));
%                 cibar(i) = cibar(i) + dt*( rbdot(j,1)*(1-stbar(i))*cibardev(i)/(rmax-rbb))...
%                 +dt*(((rturb(i)*cibar(i-1)*Uturb(i-1)*Hturb(i-1))-(rturb(i+1)*cibar(i)*Hturb(i)*Uturb(i))-(vsm*ro*cibar(i)*((rmax-rbb)/Ns)*(rturb(i)+rturb(i+1))/2)) / (Hturb(i)*((rmax-rbb)/Ns)*(rturb(i)+rturb(i+1))/2));
                    cibar(i) = cibar(i) +  (rbdot(j,1)*(1-stbar(i))*cibardev(i)/(rmax-rbb))*dt+...
                                            dt/Hturb(i)*(-vsm*ro*cibar(i)-(cibar(i)-cibar(i-1))*Uturb(i)*Hturb(i)/((rmax-rbb)*dstbar));
                            
            end
        end
        
              for i = 1:1: Ns+1  % line 247 : Exner equation to elevation change
            jj = N + 1 + i;
%             qm(i)=-(Uturb(i+1)*Hturb(i+1)*cibar(i+1)-Uturb(i)*Hturb(i)*cibar(i));
            qm(i)=cibar(i)*Uturb(i)*Hturb(i);
%             qm(i)=0; % for retention rate = 0 case !
%             @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
%             deta(i) = deta(i)  + (1 / (1 - lpm) * vsm * ro * qm(i) / Uturb(i) / Hturb(i) * dt); 
              ...
              % 3d exner turbidity :thetaP(r-rt), turbidity current width
%               etaturb(i) = etaturb(i) + rbdot(j,1)*(1-stbar(i))/(rmax-rbb)*etatdev * dt; % term 2
%               etaturb(i) = etaturb(i) + ro*vsm*thetaP*qm(i)*(rbb+stbar(i)*(rmax-rbb)-rt(i))/(1-lpm)/Uturb(i)/Hturb(i)/thetaF/(rbb+stbar(i)*(rmax-rbb))*dt; % term 1
        
              % 3d exner turbidity : instead of thetaP(r-rt), Bc (river
              % width)
              etaturb(i) = etaturb(i) + (ro*vsm*qm(i)*Bc/Uturb(i)/Hturb(i)/(1-lpm)/thetaF/(r(N+1+i)) + rbdot(j,1)*(1-stbar(i))/(rmax-rbb)*etatdev(i) )*dt;
            % r = rbb + (rmax-rbb)/Ns*i 
              deta(i)=(ro*vsm*qm(i)*Bc/Uturb(i)/Hturb(i)/(1-lpm)/thetaF/(r(N+1+i)) + rbdot(j,1)*(1-stbar(i))/(rmax-rbb)*etatdev(i) )*dt;
              eta(jj) = etaturb(i);
        ...
        etabot = etaturb(1);
        etabdot(j,1) = (etabot - etabotold) / dt;   % always positive in purely depositional
        rbdot(j,1) = rsdot(j,1) + 1 / Sa * (etasdot(j,1) - etabdot(j,1));
...
% % Dimensionless shoreline fbb figure
 ...
 figure, plot(shoreline./Lscale, timeplot./Tscale,'-k');
...
%% wash load cal
washload=washload/((qmo*dt*Ntoprint*Nprint)); % advection settling reach
